...
export function Apps() {
  ...
  const initial = useMemo<{
    ...
    return {
      ...
      type: (typeParam as AppType | null) ?? 'all',
      filter: filterParam ?? '',
    }
  }, [searchParams])

  const [sort, setSort] = useState<'asc' | 'desc'>(initial.sort)
  const [appType, setAppType] = useState<AppType>(initial.type)
  const [searchTerm, setSearchTerm] = useState(initial.filter)
  ...
  const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    updateUrl({ filter: e.target.value || undefined, page: undefined })
  }
