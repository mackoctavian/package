...
import { useCategorySalesReport } from '@dashboard/hooks/use-category-sales-report'
...
export function CategorySalesReport() {
  ...
  const { data, isLoading, isError, error } = useCategorySalesReport({
    ...
    <>
      ...
      <Main className='flex flex-1 flex-col gap-6'>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>Category sales</h1>
            <p className='text-muted-foreground text-sm'>
              Track performance across your product categories and identify top movers.
            </p>
          </div>
        ...
        {loadError ? (
          <div className='text-destructive rounded-lg border border-destructive/40 bg-destructive/10 p-4 text-sm'>
            Failed to load category sales report: {loadError}
          </div>
        ) : initialLoad ? (
          <div className='flex min-h-[480px] items-center justify-center rounded-xl border bg-background'>
            <LoadingIndicator label='Loading category sales report…' />
          </div>
        ) : (
          <>
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
              ...
              <SummaryCard
                ...
                description='Units sold across categories'
              />
            </div>
            ...
            <Card>
              <CardHeader className='flex flex-col gap-1 md:flex-row md:items-center md:justify-between'>
                <div>
                  <CardTitle>Sales trend by category</CardTitle>
                  <CardDescription>
                    {data?.summary
            ...
            <Card>
              <CardHeader className='flex flex-col gap-1 md:flex-row md:items-center md:justify-between'>
                <div>
                  ...
                  <CardDescription>Detailed performance for each category.</CardDescription>
                </div>
              </CardHeader>
              <CardContent className='pt-0'>
                {loading ? (
                    ...
                    <TableBody>
                      {data.categories.map((category) => (
                        <TableRow key={category.id}>
                          ...
                          <TableCell className='text-right'>{formatCurrency(category.discounts)}</TableCell>
                  ...
                  <div className='text-muted-foreground flex h-40 items-center justify-center text-sm'>No categories found for the selected filters.</div>
                )}
              </CardContent>
            </Card>
          </>
