...
import { useDepartmentSalesReport } from '@dashboard/hooks/use-department-sales-report'
...
export function DepartmentSalesReport() {
  ...
  const { data, isLoading, isError, error } = useDepartmentSalesReport({
    ...
    <>
      ...
      <Main className='flex flex-1 flex-col gap-6'>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>Department sales</h1>
            <p className='text-muted-foreground text-sm'>
              Compare department-level performance and see which areas drive the most revenue.
            </p>
          </div>
        ...
        {loadError ? (
          <div className='text-destructive rounded-lg border border-destructive/40 bg-destructive/10 p-4 text-sm'>
            Failed to load department sales report: {loadError}
          </div>
        ) : initialLoad ? (
          <div className='flex min-h-[480px] items-center justify-center rounded-xl border bg-background'>
            <LoadingIndicator label='Loading department sales report…' />
          </div>
        ) : (
          <>
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
              ...
              <SummaryCard
                ...
                description='Units sold across departments'
              />
            </div>
            ...
            <Card>
              <CardHeader className='flex flex-col gap-1 md:flex-row md:items-center md:justify-between'>
                <div>
                  <CardTitle>Sales trend by department</CardTitle>
                  <CardDescription>
                    {data?.summary
            ...
            <Card>
              <CardHeader className='flex flex-col gap-1 md:flex-row md:items-center md:justify-between'>
                <div>
                  ...
                  <CardDescription>Detailed performance for each department.</CardDescription>
                </div>
              </CardHeader>
                    ...
                    <TableBody>
                      {data.departments.map((department) => (
                        <TableRow key={department.id}>
                          ...
                          <TableCell className='text-right'>{formatCurrency(department.grossSales)}</TableCell>
                          <TableCell className='text-right'>{formatCurrency(department.discounts)}</TableCell>
                          <TableCell className='text-right'>{formatCurrency(department.netSales)}</TableCell>
                          <TableCell className='text-right'>{formatNumber(department.quantity)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ) : (
                  <div className='text-muted-foreground flex h-40 items-center justify-center text-sm'>No department performance data for the selected filters.</div>
                )}
              </CardContent>
            </Card>
          </>
